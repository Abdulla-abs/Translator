...
- **增量 / 全量双模式**：首页**全量**在上、**增量**在下；二者均先上传源文件创建项目卡片（卡片标注「全量」/「增量」）；在编辑器详情页上传目标 `strings.xml` 后方可翻译与导出
...
- **项目持久化**：上传/最近项目保存在本地目录（`source.xml`、`target-baseline.xml`（增量）、`result.xml`、`session.json`），退出应用后可恢复会话与错误状态
- **导出**：翻译完成后导出 XML；存在失败条目时会提示是否继续导出（失败条目保留源文）
- **文件浏览**：浏览本地目录并打开 `strings.xml` 进入编辑器
- **主题与设置**：多套 Material 3 外观、默认语言、强制重译等（合并策略由项目模式自动绑定：增量→仅补全，全量→覆盖）
- **界面语言**：应用 UI 支持英文与简体中文（与业务翻译的源/目标语言相互独立）
...
2. **界面语言（可选）**：在 **设置 → 翻译策略** 中选择 **界面语言**（`English` 或 `简体中文`）。仅影响应用菜单、对话框与标签，不会改变项目 `strings.xml` 的翻译源/目标语言。
3. **导入 XML**：在 Dashboard 上传 `strings.xml`，或在 Files 页浏览本地文件打开。
4. **选择语言**：在文件编辑页选择源语言与目标语言（可与 Android `values-xx` 目录语言码对应，如 `en`、`zh`、`zh-rTW`）。
5. **开始翻译**：点击开始翻译；可在条目列表中查看状态，失败条目可单独重试。
6. **导出**：全部成功后可导出；若存在错误条目，导出前会弹出确认对话框。
